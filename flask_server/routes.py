...
def new_post():
    ...
    if form.validate_on_submit():
        if form.picture.data :
            ...
            mask, msi, rgb = predict(file_path)
            post = Post(title= form.title.data, content=form.content.data, mask=save_picture_post(mask, "mask.png"),
            msi=save_picture_post(msi, "msi.png"), rgb=save_picture_post(rgb, "rgb.png"), author= current_user)
...
def zoom(post_id, zoom_size):
    ...
    if post.author != current_user:
        abort(403)
    return render_template('viz.html', title=post.title, zoom=zoom_size, post = post)
...
def viz(post_id):
    ...
    if post.author != current_user:
        abort(403)
    return render_template('viz.html', title=post.title, zoom=3, post = post)
