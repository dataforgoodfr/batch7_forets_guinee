...
from flask_server.model_sim  import generate
...
def new_post():
    ...
    if form.validate_on_submit():
        if form.picture.data :
            ...
            mask, msi, rgb, infra, mask_msi, mask_rgb, msi_rgb, mask_infra, rgb_infra, msi_infra, mask_msi_infra, mask_rgb_infra, msi_rgb_infra, msi_rgb_mask, all, kpis = generate(file_path)
...
def viz_zoom_img(post_id, zoom_size, zoom_id):
    ...
    if post.author != current_user:
        abort(403)
    zoom_msi_style, zoom_mask_style, zoom_rgb_style, zoom_infra_style, _, _, path_zoom = get_imgs_with_id(post, zoom_id)
    msi_style, mask_style, rgb_style, infra_style, mask, msi, path_img = get_imgs_with_id(post, img_id)
    return render_template('viz.html', title=post.title, img_path = path_img, zoom_path = path_zoom, msi = msi, mask= mask,  rgb_style=rgb_style, msi_style=msi_style, mask_style=mask_style,infra_style=infra_style,  zoom_msi_style = zoom_msi_style, zoom_mask_style=zoom_mask_style, zoom_rgb_style = zoom_rgb_style, zoom_infra_style = zoom_infra_style, img = img_id % 16, zoom_id = zoom_id %16 , zoom=zoom_size, post = post)
...
def viz_zoom(post_id, zoom_size):
    ...
    if post.author != current_user:
        abort(403)
    return render_template('viz.html', title=post.title, img_path = url_for ('static', filename= 'post_picture/' + post.msi), zoom_path = url_for ('static', filename= 'post_picture/' + post.msi), msi = 6, mask= 1, rgb_style= "", msi_style="text-primary", mask_style="", infra_style="",  zoom_msi_style = "text-primary", zoom_mask_style="", zoom_rgb_style = "", zoom_infra_style = "", img = 2, zoom_id = 2, zoom=zoom_size, post = post)
...
def viz_img_zoom(post_id, zoom_size, img_id):
    ...
    if post.author != current_user:
        abort(403)
    msi_style, mask_style, rgb_style, infra_style, mask, msi, path_img = get_imgs_with_id(post, img_id)
    return render_template('viz.html', title=post.title, img_path = path_img, zoom_path = path_img, msi = msi, mask= mask,  rgb_style=rgb_style, msi_style=msi_style, mask_style=mask_style, infra_style=infra_style,zoom_msi_style = msi_style, zoom_mask_style=mask_style, zoom_rgb_style = rgb_style, zoom_infra_style = infra_style, img = img_id % 16, zoom_id = img_id %16, zoom=zoom_size, post = post)
